...
export function KategorienDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: KategorienDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kategorie_name": string | null, // Kategoriename\n  "kategorie_beschreibung": string | null, // Beschreibung\n  "kategorie_farbe": LookupValue | null, // Farbe (select one key: "blau" | "gruen" | "gelb" | "orange" | "lila" | "grau" | "rot") mapping: blau=Blau, gruen=Grün, gelb=Gelb, orange=Orange, lila=Lila, grau=Grau, rot=Rot\n}`;
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="kategorie_farbe">Farbe</Label>
            <Select
              value={lookupKey(fields.kategorie_farbe) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, kategorie_farbe: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="kategorie_farbe"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
